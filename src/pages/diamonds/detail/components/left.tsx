...
const Left = (props: LeftProps) => {
  ...
  const [mediaList, setMediaList] = useState<Media[]>(props.mediaList);
  useEffect(() => {
    ...
    <div>
      ...
      <div className="w-full h-fit m-3 p-4">
        ...
        <div className="cert flex flex-row justify-between  font-light text-sm ">
          ...
          <p>{getEnumStringFromNumber(diamond.Fluorescence, Fluorescence)}</p>
        </div>
        <hr className="my-2" />
        <div className="cert flex flex-row justify-between  font-light text-sm ">
...
export function ImageDisplay(params: ImageDisplayProps): React.ReactNode {
  const [currentImage, setCurrentImage] = useState<Media>(null);
  ...
  useEffect(() => {
    if (params.images && params.images.length > 0) {
      setCurrentImage(params.images[0]);
    }
  }, [params]);
  return (
